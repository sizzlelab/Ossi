...
#    self.t = e32.Ao_timer()
  ...
  def start(self):
    ...
    positioning.position(course=1,satellites=1, callback=self.run, interval=10000000, partial=1)
    self.running = True
  ...
  def run(self,pos):
    ...
    wlan_data = self.get_wlan_data()
    if wlan_data:
      position["wlan_data"] = wlan_data
      self.text += "\nWLANs found: "+str(len(position["wlan_data"]["wlanlist"]))+"\n"
    self.track.append(position);
    self.position = position
    if self.extra_handler != None:
      self.extra_handler()
